...
type Props = {
  ...
  onSelectTrip: (trip: TripSelection) => void;
  onClearTrip: () => void;
};
...
function center(stop: Stop): [number, number] | null {
  if (!stop.coordinates.length) return null;
  const lng = stop.coordinates.reduce((sum, c) => sum + c[0], 0) / stop.coordinates.length;
  const lat = stop.coordinates.reduce((sum, c) => sum + c[1], 0) / stop.coordinates.length;
  return [lat, lng];
}
...
export default function MapView({ stops, selectedTrip, onSelectStop, onSelectTrip, onClearTrip }: Props) {
  ...
  useEffect(() => {
    ...
    async function loadVehicles() {
      try {
        const response = await fetch(`${API}/fahrzeuge`, { cache: "no-store" });
        if (!response.ok) throw new Error();
        const data = await response.json() as { features?: VehicleFeature[] };
        if (cancelled) return;
        vehiclesRef.current = data.features || [];
        setVehicleCount(vehiclesRef.current.length);
        setMapError("");
        renderVehicles();
      } catch {
        if (!cancelled) setMapError("Die Live-Positionen sind gerade nicht verfügbar.");
      }
    }
    ...
    function renderVehicles() {
      ...
      vehiclesRef.current.forEach((feature) => {
        ...
        const tripId = String(p.fahrtbezeichner || "");
        if (!coords || !tripId) return;
        if (selectedTrip && tripId !== selectedTrip.id) return;
        const line = String(p.linientext || "Bus");
        const point: [number, number] = [coords[1], coords[0]];
        const icon = L.divIcon({
          className: "",
          html: `<div class="mapBus${selectedTrip ? " selected" : ""}" style="background:${color(line)}">${line}</div>`,
          iconSize: [38, 32],
          iconAnchor: [19, 16]
        });
    ...
    loadVehicles();
    const timer = window.setInterval(loadVehicles, 20000);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await fetch(`${API}/fahrten/${encodeURIComponent(selectedTrip.id)}`, { cache: "no-store" });
        if (!response.ok) throw new Error();
        const trip = await response.json() as TripFeature;
        if (cancelled) return;
        const coords = trip.geometry?.coordinates || [];
        if (!coords.length) return;
        const vehicle = vehiclesRef.current.find((f) => String(f.properties?.fahrtbezeichner || "") === selectedTrip.id);
        const v = vehicle?.geometry?.coordinates;
        let split = 0;
        if (v) {
          let best = Infinity;
          coords.forEach((c, index) => {
            const d = (c[0] - v[0]) ** 2 + (c[1] - v[1]) ** 2;
            if (d < best) { best = d; split = index; }
          });
        }
        ...
        if (!cancelled) setMapError("Der Fahrtweg konnte gerade nicht geladen werden.");
      }
    })();
  ...
  function locate() {
    ...
    navigator.geolocation.getCurrentPosition(({ coords }) => {
      ...
      L.circleMarker(point, { radius: 7, color: "#fff", weight: 3, fillColor: "#1686ff", fillOpacity: 1 }).addTo(map);
    }, () => setMapError("Standort nicht freigegeben."), { enableHighAccuracy: true, timeout: 10000, maximumAge: 60000 });
  }

  return <section className="mapCard">
    <div className="mapHeader">
      <div>
        <p className="eyebrow">Münster · live</p>
        <h1>{selectedTrip ? `Linie ${selectedTrip.line}` : "Live-Karte"}</h1>
        <p>{selectedTrip ? `Richtung ${selectedTrip.direction}` : `${vehicleCount} Busse live · Haltestellen ab Zoom 14`}</p>
      </div>
      <button className="round" onClick={locate} aria-label="Zu meinem Standort"><span className="locateDot">◎</span></button>
    </div>
    ...
    <div className="mapLegend">
      {selectedTrip ? <><span><i className="legendPast"/> gefahren</span><span><i className="legendFuture" style={{ background: color(selectedTrip.line) }}/> kommt noch</span></> : <span>Bus oder Haltestelle antippen</span>}
      {mapError && <span className="mapError">{mapError}</span>}
    </div>
  </section>;
}
